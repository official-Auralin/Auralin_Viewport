...
StaticPopupDialogs["AURALIN_VIEWPORT_RESET_PROFILE_CONFIRM"] = StaticPopupDialogs["AURALIN_VIEWPORT_RESET_PROFILE_CONFIRM"] or {
    ...
    button2 = CANCEL,
    OnAccept = function(_, profileName)
        if not AuralinVP.ResetProfile then
            return
        end

        local success, result = AuralinVP:ResetProfile(profileName)
        if not success then
            AuralinVP:Print(result or "Unable to reset profile.")
            return
        end

        AuralinVP:Print("Reset profile '" .. tostring(result) .. "' to default values.")
    end,
    timeout = 0,
    whileDead = true,
    hideOnEscape = true,
